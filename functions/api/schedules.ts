// GET /api/schedule?date=YYYY-MM-DD&user=<optional>
// POST /api/schedule  { username, date, blocks:[{status,start,end}] }
...
export const onRequestGet: PagesFunction = async ({ request, env }) => {
  ...
  const r = await fetch(`${env.AUTH_BASE}/schedules${qs}`, { headers: { Cookie: `access_token=${token}` }});
  return pass(r);
};
export const onRequestPost: PagesFunction = async ({ request, env }) => {
  const token = readCookie(request, "allstar_at"); if (!token) return json({ error: "NO_SESSION" }, 401);
  const body = await request.text();
  const r = await fetch(`${env.AUTH_BASE}/schedules`, { method:"POST", headers:{ "content-type":"application/json", Cookie:`access_token=${token}` }, body });
  return pass(r);
};
